...
Client.prototype._format_value = function (v) {
  if (Buffer.isBuffer(v)) {
    ...
    // The following line is a custom modification to this library by
    // gabe to support binary-encoded buffers in mariaDB queries. Without this
    // modification, it is impossible to send binary-encoded data to MariaDB
    // through this client because the utf8 encoding step mangles the binary
    // data.
    //
    // See the branch of node-mariasql 0.1.21 with this change at
    // https://github.com/navahq/node-mariasql/tree/gs-add-buffer-to-old-version
    //
    // See the branch of the most recent node-mariasql with this change at
    // https://github.com/navahq/node-mariasql/tree/gs-buffer
    //
    // See the pull request for the root repo of node-maria sql at
    // https://github.com/mscdex/node-mariasql/pull/147
    return "X'" + addon.escape(v.toString('hex')) + "'";
  }
